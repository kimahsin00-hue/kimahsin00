...
def init_db():
    ...
    cursor.execute('''CREATE TABLE IF NOT EXISTS weekly_dm_content (id INTEGER PRIMARY KEY CHECK(id=1), title TEXT, description TEXT, footer TEXT)''')
    conn.commit()
    conn.close()
...
# ==========================================
CAPHRAS_DB = {
    # ── 주무기 (동 우두머리 주무기/각성/보조) ──────────────────────────────
    # tuple: (단계별 소모량, 누적 소모량, 스탯증가)  ← txt 기준 검수 완료
    "녹색무기": {"동": {
        0:  (0,    0,     0),
        1:  (297,  297,   1),
        2:  (389,  686,   2),
        ...
        async with session.get(url, params=params, timeout=aiohttp.ClientTimeout(total=15)) as response:
            ...
            if isinstance(data, dict):
                for wrap_key in ("data", "items", "result", "list", "subList", "content"):
                    ...
                    price_keys = ("basePrice", "base_price", "pricePerOne", "price",
                                  "sid", "currentStock", "count", "stock", "minEnhance")
                    if any(k in data for k in price_keys):
                        data = [data]
                    else:
                        raise RuntimeError(f"arsha API 응답이 예상 못한 dict 구조. keys={list(data.keys())}")
            ...
            for row in data:
                try:
                    min_enh = (row.get("sid") or row.get("subKey") or row.get("minEnhance") or row.get("enhance") or 0)
                    price = (row.get("basePrice") or row.get("pricePerOne") or row.get("price") or row.get("minPrice") or row.get("base_price") or 0)
                    stock = (row.get("count") or row.get("currentStock") or row.get("stock") or row.get("current_stock") or 0)
                    trades = (row.get("totalTrades") or row.get("tradeCount") or row.get("soldCount") or row.get("total_trades") or 0)
                    entries.append({
                        "item_id": int(row.get("mainKey") or row.get("id") or item_id),
                        "min_enhance": int(min_enh),
                        "max_enhance": int(row.get("maxEnhance") or min_enh),
                        "base_price": int(price),
                        "current_stock": int(stock),
                        "total_trades": int(trades),
                        "last_sale_price": int(row.get("lastSoldPrice") or row.get("last_sale_price") or 0),
                    })
...
async def fetch_pa_sublist(item_id):
    ...
    async with aiohttp.ClientSession(headers=PA_HEADERS) as session:
        async with session.post(url, json=payload, timeout=8) as response:
            ...
            try:
                text = raw.decode("utf-16", errors="surrogatepass")
...
# ==========================================
# 군왕 무기 계산기 - 아이템 ID 정의 및 와푸봇 UI
# ==========================================
SOV_WEAPON_CATEGORIES = {
    "동 검은별 주무기":     (1, 1),   
    "동 크자카 주무기":     (1, 1),   
    "동 검은별 각성무기":   (1, 5),   
    "동 단델리온 각성무기": (1, 5),   
...
SOV_WEAPON_NAME_PATTERNS = {
    "동 검은별 주무기":     (["검은별"], ["각성", "투구", "갑옷", "장갑", "신발",
                                          "방패", "단검", "노리개", "장식매듭", "타리스만",
                                          "완갑", "각궁", "고검", "라곤", "수리검", "표창", "금계", "비초아리", "크라톤"]),
    "동 크자카 주무기":     (["크자카"], ["방패", "단검", "노리개", "장식매듭", "타리스만",
                                          "완갑", "각궁", "고검", "라곤", "수리검", "표창", "금계", "비초아리", "크라톤",
                                          "추종세력"]),
    "동 검은별 각성무기":   (["검은별"], ["투구", "갑옷", "장갑", "신발",
                                          "방패", "단검", "노리개", "장식매듭", "타리스만",
                                          "완갑", "각궁", "고검", "라곤", "수리검", "표창", "금계", "비초아리", "크라톤",
                                          "장검", "장궁", "부적", "도끼", "소검", "도검", "지팡이", "태도", "권갑", "반월추", "석궁", "플로랑", "전투도끼", "사곡도"]),
    "동 단델리온 각성무기": (["단델리온"], []),
    "동 검은별 보조무기":   (["검은별"], ["각성", "투구", "갑옷", "장갑", "신발",
                                          "장검", "장궁", "부적", "도끼", "소검", "도검", "지팡이", "태도", "권갑", "반월추", "석궁", "플로랑", "전투도끼", "사곡도"]),
    "동 누베르 보조무기":   (["누베르"], ["조각상"]),
    "동 쿠툼 보조무기":     (["쿠툼"],   ["조명들"]),
}
...
SOV_ITEM_IDS = {
    ...
    "황혼의 보석":          {"id": 767077, "sid": 0},
    "태초의 보석":          {"id": 821247, "sid": 0},
}

SOV_WEAPON_KEYS = {"동 검은별 주무기", "동 크자카 주무기", "동 검은별 각성무기",
                   "동 단델리온 각성무기", "동 검은별 보조무기", "동 누베르 보조무기", "동 쿠툼 보조무기"}

# 🚨 수정됨: 카프라스 20단 비용 누락 수정 완료 (보스 무기에 29403개 추가)
SOV_RECIPES = {
    "주무기": [
        ("동 검은별 주무기 + 동 검은별 주무기",
         [("동 검은별 주무기", 2)]),
        ("동 검은별 주무기 + 동 크자카 주무기 + 황혼의 보석",
         [("동 검은별 주무기", 1), ("동 크자카 주무기", 1), ("황혼의 보석", 1), ("카프라스의 돌", 29403)]),
    ],
    "각성무기": [
        ("동 검은별 각성무기 + 동 검은별 각성무기",
         [("동 검은별 각성무기", 2)]),
        ("동 검은별 각성무기 + 동 단델리온 각성무기 + 황혼의 보석",
         [("동 검은별 각성무기", 1), ("동 단델리온 각성무기", 1), ("황혼의 보석", 1), ("카프라스의 돌", 29403)]),
    ],
    "보조무기": [
        ("동 검은별 보조무기 + 동 검은별 보조무기 + 황혼의 보석",
         [("동 검은별 보조무기", 2), ("황혼의 보석", 1)]),
        ("동 검은별 보조무기 + 동 누베르/쿠툼 보조무기",
         [("동 검은별 보조무기", 1), ("동 쿠툼 보조무기", 1), ("황혼의 보석", 1), ("카프라스의 돌", 29403)]),
        ("동 검은별 + 황혼의 보석 + 태초의 보석",
         [("동 검은별 보조무기", 1), ("황혼의 보석", 1), ("태초의 보석", 1)]),
    ],
}

async def get_sov_weapon_price(item_key: str) -> tuple:
    requires, excludes = SOV_WEAPON_NAME_PATTERNS.get(item_key, ([], []))
    if not requires:
        return (0, 0, 0)
    ...
    for name, iid in ITEM_LIST.items():
        if excludes and any(ex in name for ex in excludes):
            continue
        if all(req in name for req in requires):
            matched_ids.append(iid)

    if not matched_ids:
        print(f"⚠️ get_sov_weapon_price: '{item_key}' 조건에 맞는 무기를 ITEM_LIST에서 찾지 못함 (requires={requires}, excludes={excludes})")
        return (0, 0, 0)

    tasks = [fetch_market_sublist(iid) for iid in matched_ids]
    results = await asyncio.gather(*tasks, return_exceptions=True)

    best_price, best_stock, best_trades = 0, 0, 0
    for entries in results:
        if isinstance(entries, Exception) or not entries:
            continue
        for entry in entries:
            if entry.get("min_enhance", 0) != 20:
                continue
            price = entry.get("base_price", 0)
            stock = entry.get("current_stock", 0)
            trades = entry.get("total_trades", 0)
            if price > 0 and (best_price == 0 or price < best_price):
                best_price, best_stock, best_trades = price, stock, trades

    return (best_price, best_stock, best_trades)


# 🚨 수정됨: 황혼의 보석 및 태초의 보석 자동 역산 처리
async def fetch_sov_prices(weapon_type: str) -> dict:
    ...
    if "황혼의 보석" in needed or "태초의 보석" in needed:
        needed.add("마력의 파편")
        needed.add("카프라스의 돌")

    async def _fetch_one(name):
        if name in ["황혼의 보석", "태초의 보석"]:
            return (0, 0, 0)
        if name in SOV_WEAPON_KEYS:
            return await get_sov_weapon_price(name)
        cfg = SOV_ITEM_IDS.get(name)
        if cfg:
            result = await get_market_price(cfg["id"], cfg["sid"])
            if result[0] == 0:
                print(f"⚠️ fetch_sov_prices: '{name}' (id={cfg['id']}) 가격 조회 결과 0")
            return result
        return (0, 0, 0)

    prices = {}
    results = await asyncio.gather(*[_fetch_one(n) for n in needed], return_exceptions=True)
    for name, result in zip(needed, results):
        prices[name] = (0, 0, 0) if isinstance(result, Exception) or result is None else result

    p_shard = prices.get("마력의 파편", (0, 0, 0))[0]
    p_caphras = prices.get("카프라스의 돌", (0, 0, 0))[0]
    calc_price = (p_shard * 100) + (p_caphras * 20000)

    if "황혼의 보석" in needed:
        prices["황혼의 보석"] = (calc_price, 0, 0)
    if "태초의 보석" in needed:
        prices["태초의 보석"] = (30000000000, 0, 0)

    return prices

# 🚨 수정됨: 와푸 봇 디자인과 100% 동일한 UI 구성
class SovResultView(discord.ui.View):
    ...
    def build_embed(self) -> discord.Embed:
        embed = discord.Embed(title=f"군왕 무기 계산({self.weapon_type})", color=0x2b2d31)
        lines = []
    ...
    async def btn_avg(self, interaction: discord.Interaction, button: discord.ui.Button):
        embed = discord.Embed(title="군왕 무기 계산기", color=0x2b2d31)
        ...
        final_order = []
        weapons = [n for n in order if "무기" in n]
        jewels = [n for n in order if "보석" in n]
        
        for w in weapons: final_order.append(w)
        if jewels:
            if "마력의 파편" not in final_order: final_order.append("마력의 파편")
            if "카프라스의 돌" not in final_order: final_order.append("카프라스의 돌")
        for j in jewels: final_order.append(j)
        for n in order:
            if n not in final_order: final_order.append(n)
            
        lines = []
        for name in final_order:
            price = self.prices.get(name, (0, 0, 0))[0]
            lines.append(f"**{name}**")
            if price == 0:
                lines.append(f"> 0 (0)\n")
            else:
                lines.append(f"> {price:,} ({format_kr(price)})\n")
...
# ==========================================
# [기능 1] 지정 채널 채팅 → 서버원 전체 DM 전송
# ==========================================
BROADCAST_CHANNEL_ID = 1506670935464345763
...
async def on_message(message: discord.Message):
    ...
    if message.channel.id == BROADCAST_CHANNEL_ID:
        ...
        embed.set_footer(text=f"보낸 사람: {message.author.display_name}")
        success, fail = 0, 0
        ...
        try:
            await message.channel.send(
                f"✅ 전체 DM 전송 완료 — 성공 {success}명 / 실패 {fail}명",
                delete_after=10
            )
        except:
            pass
...
# ==========================================
# [기능 2] /설치-쿠폰 → 등록 버튼 → 쿠폰번호 입력 → 서버원 전체 DM 전송
# ==========================================
...
class CouponPanelView(discord.ui.View):
    ...
    @discord.ui.button(label="쿠폰 등록", style=discord.ButtonStyle.success, custom_id="coupon_register_btn")
    async def btn_register(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_modal(CouponInputModal())


@bot.tree.command(name="설치-쿠폰", description="쿠폰 등록 패널 설치 (등록 버튼 → 전체 DM 전송)")
@discord.app_commands.default_permissions(administrator=True)
async def setup_coupon(interaction: discord.Interaction):
    embed = discord.Embed(
        title="🎁 쿠폰 등록",
        description="아래 버튼을 눌러 쿠폰 번호를 등록하면 서버원 전체에게 DM으로 전달됩니다.",
        color=0x2ecc71
    )
    await interaction.channel.send(embed=embed, view=CouponPanelView())
    await interaction.response.send_message("설치 완료", ephemeral=True)


# ==========================================
# [기능 3] 매주 토요일 오후 6시 KST 전체 DM + /내용수정 명령어
# ==========================================
@tasks.loop(minutes=1)
async def weekly_saturday_dm():
    now = datetime.now(KST)
    # 토요일(weekday==5), 18:00, 0초
    if now.weekday() != 5 or now.hour != 18 or now.minute != 0:
        return

    conn = sqlite3.connect('bdo_data.db')
    c = conn.cursor()
    c.execute("SELECT title, description, footer FROM weekly_dm_content WHERE id=1")
    row = c.fetchone()
    conn.close()

    if not row or not row[0]:
        print("⚠️ 주간 DM: /내용수정 으로 내용을 먼저 설정해주세요.")
        return
    ...
    for guild in bot.guilds:
        ...
        for member in guild.members:
            ...
            try:
                ...
                fail += 1
        print(f"[주간 토요일 DM] 서버={guild.name} 성공={success} 실패={fail}")
...
@bot.tree.command(name="내용수정", description="[관리자] 매주 토요일 오후 6시에 전송할 DM 내용을 수정합니다")
@discord.app_commands.default_permissions(administrator=True)
async def edit_weekly_dm(interaction: discord.Interaction):
    await interaction.response.send_modal(WeeklyDmEditModal())
...
DUMP_MAX_RETRIES = 3
DUMP_RETRY_DELAY_SEC = 10 
DUMP_BACKUP_FRESH_HOURS = 24 
...
async def on_ready():
    ...
    if not weekly_saturday_dm.is_running(): weekly_saturday_dm.start()
