...
STATUS_TITLES = {"blessing": "영물의 축복", "edana": "에다니아"}
...
class StatusManualTimeModal(discord.ui.Modal, title="시간 직접 입력"):
    minutes_input = discord.ui.TextInput(label='남은 시간(분)', placeholder='예: 45', required=True, max_length=4)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            await interaction.response.send_message("남은 시간은 1 이상의 숫자(분)로 입력해주세요.", ephemeral=True); return
        await finalize_status_report(interaction, self.s_type, self.kind, self.server, minutes)
...
def build_status_view_embed(s_type, s_title, rows):
    """현황 임베드를 만든다. rows: [(content, timestamp, expire_time), ...]. 분 단위로 자동 갱신되는 '남은 시간'을 매번 새로 계산."""
    embed = discord.Embed(title=f"최신 {s_title} 현황", color=0x9b59b6)
    now = datetime.now(KST)
    if not rows:
        embed.description = "아직 등록된 제보가 없습니다."
        return embed
    ...
    for content, timestamp_str, expire_str in rows:
        ...
        if expire_str:
            ...
            remain_min = int((expire_time - now).total_seconds() // 60)
            if remain_min > 0:
                remain_text = f"⏳ {remain_min}분 남음"
            else:
                remain_text = "⚠️ 만료됨"
        else:
            remain_text = "-"
        ...
        desc += f"**{kind}** | {server}\n• {remain_text}\n\n"
    embed.description = desc
    embed.set_footer(text=f"갱신 시각: {now.strftime('%H:%M:%S')} · 새로고침은 '현황확인'을 다시 눌러주세요")
    return embed
...
class StatusViewPanel(discord.ui.View):
    ...
    async def btn_refresh_callback(self, interaction: discord.Interaction):
        conn = sqlite3.connect('bdo_data.db')
        c = conn.cursor()
        # 만료된 지 30분 넘은 오래된 제보는 목록에서 제외(화면이 만료 항목으로 도배되지 않도록)
...
@discord.app_commands.choices(종류=[
    discord.app_commands.Choice(name="영물의 축복", value="blessing"),
...
async def check_status_notify_channel(interaction: discord.Interaction):
    ...
    conn.close()
    lines = []
    for s_type, label in [("blessing", "영물의 축복"), ("edana", "에다니아")]:
        if s_type in rows:
