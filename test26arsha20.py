...
NOTICE_CHANNEL_ID = 1521970730181464125
...
async def on_message(message):
    ...
    embed.set_footer(text=f"작성자: {message.author.display_name} | {guild.name}")
    
    # 이미지나 첨부파일이 있다면 임베드에 추가
    if message.attachments:
        embed.set_image(url=message.attachments[0].url)

    await message.channel.send("모든 유저에게 DM 공지 발송을 시작합니다. (유저 수에 따라 시간이 걸릴 수 있습니다.)", delete_after=5.0)

    success_count = 0
    fail_count = 0

    # 2. 모든 유저에게 순차 발송 (레이트 리밋 방지)
    for member in guild.members:
        ...
        try:
            ...
            success_count += 1
            # 🛑 중요: 디스코드 API 차단 방지를 위해 발송당 0.2~0.5초 쉼
            await asyncio.sleep(0.3) 
        except discord.Forbidden:
            # 유저가 서버 DM을 차단해둔 경우 예외 처리
            fail_count += 1
        except Exception as e:
            fail_count += 1

    await message.channel.send(f"공지 발송 완료!\n- 성공: {success_count}명\n- 실패(DM 차단 등): {fail_count}명", delete_after=5.0)
    
    # 다른 명령어들도 정상 작동하도록 처리
    await bot.process_commands(message)
