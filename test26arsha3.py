...
def init_db():
    ...
    cursor.execute("PRAGMA table_info(rift_history)")
    rift_cols = [row[1] for row in cursor.fetchall()]
    required_rift_cols = {"id", "reporter_id", "reporter_name", "kill_time"}
    if not required_rift_cols.issubset(set(rift_cols)):
        # 예전 스키마(예: user_id, boss_name 기반 다중 행 구조)가 남아있는 DB를 안전하게 새 구조로 이전
        cursor.execute("ALTER TABLE rift_history RENAME TO rift_history_old")
        cursor.execute('''CREATE TABLE rift_history (id INTEGER PRIMARY KEY CHECK(id=1), reporter_id INTEGER, reporter_name TEXT, kill_time DATETIME)''')
        try:
            # 예전 테이블에서 가장 최근 처치 시간 하나만 살려서 새 구조로 옮긴다 (있으면 보존, 없으면 건너뜀)
            cursor.execute("PRAGMA table_info(rift_history_old)")
            old_cols = [row[1] for row in cursor.fetchall()]
            if "kill_time" in old_cols:
                cursor.execute("SELECT kill_time FROM rift_history_old ORDER BY kill_time DESC LIMIT 1")
                row = cursor.fetchone()
                if row:
                    cursor.execute("INSERT INTO rift_history (id, reporter_id, reporter_name, kill_time) VALUES (1, NULL, NULL, ?)", (row[0],))
        except Exception:
            pass
        cursor.execute("DROP TABLE rift_history_old")
    ...
    cursor.execute("PRAGMA table_info(status_reports)")
    existing_cols = [row[1] for row in cursor.fetchall()]
    if "expire_time" not in existing_cols:
        cursor.execute("ALTER TABLE status_reports ADD COLUMN expire_time DATETIME")
...
BOSS_DB = {
    "월": [
        ("02:00", "크자카, 불가살"), ("11:00", "누베르, 우투리"),
        ("14:00", "가모스"), ("16:00", "쿠툼, 금돼지왕"),
        ("20:00", "카란다, 산군"), ("23:15", "가모스"), ("23:30", "오핀, 불가살"),
    ],
    "화": [
        ("02:00", "누베르, 우투리"), ("11:00", "쿠툼, 금돼지왕"),
        ("14:00", "가모스"), ("16:00", "누베르, 산군"),
        ("20:00", "크자카, 불가살"), ("23:15", "가모스"), ("23:30", "카란다, 우투리"),
    ],
    "수": [
        ("02:00", "오핀, 금돼지왕"), ("14:00", "가모스"),
        ("16:00", "카란다, 산군"), ("19:00", "권트, 무라카"),
        ("20:00", "쿠툼, 불가살"), ("23:15", "가모스"), ("23:30", "크자카, 우투리"),
    ],
    "목": [
        ("00:15", "벨"), ("02:00", "카란다, 금돼지왕"),
        ("11:00", "크자카, 산군"), ("14:00", "가모스"),
        ("16:00", "누베르, 불가살"), ("20:00", "누베르, 우투리"),
        ("23:15", "가모스"), ("23:30", "쿠툼, 금돼지왕"),
    ],
    "금": [
        ("02:00", "쿠툼, 산군"), ("11:00", "카란다, 불가살"),
        ("14:00", "가모스"), ("16:00", "크자카, 우투리"),
        ("20:00", "누베르, 금돼지왕"), ("23:15", "가모스"), ("23:30", "오핀, 산군"),
    ],
    "토": [
        ("02:00", "쿠툼, 불가살"), ("11:00", "카란다, 우투리"),
        ("14:00", "가모스"), ("16:00", "크자카, 금돼지왕"),
        ("17:00", "검은그림자"), ("19:00", "권트, 무라카"),
        ("23:30", "누베르, 금돼지왕"),
    ],
    "일": [
        ("00:15", "가모스"), ("02:00", "누베르, 산군"),
        ("11:00", "쿠툼, 불가살"), ("14:00", "가모스"),
        ("16:00", "카란다, 우투리"), ("17:00", "벨"),
        ("20:00", "크자카, 산군"), ("23:15", "가모스"), ("23:30", "누베르, 금돼지왕"),
    ],
}
...
ARSHA_BASE_URL = "https://api.arsha.io/v2/kr"
ARSHA_HEADERS = {"User-Agent": "Mozilla/5.0", "Accept": "application/json"}

async def fetch_arsha_sublist(item_id):
    """
    arsha.io 가격 API로 특정 item_id의 강화단계별(sid) 가격 리스트를 가져온다.
    펄어비스 원본(PA) API와 응답 형식이 달라서(파이프 구분 문자열 X, JSON 배열),
    fetch_pa_sublist()와 동일한 dict 형태로 변환해서 반환한다.
    ⚠️ arsha.io 측 응답 필드명이 문서/실제가 다를 수 있어 여러 후보 키를 함께 확인한다.
    """
    url = f"{ARSHA_BASE_URL}/GetWorldMarketSubList"
    params = {"id": item_id, "lang": "kr"}
    async with aiohttp.ClientSession(headers=ARSHA_HEADERS, connector=aiohttp.TCPConnector(ssl=False)) as session:
        async with session.get(url, params=params, timeout=aiohttp.ClientTimeout(total=8)) as response:
            ...
            # arsha.io 응답이 배열이 아니라 dict로 오는 경우를 흡수
            # (1) {"data": [...]} / {"items": [...]} / {"result": [...]} 처럼 감싸진 경우
            # (2) sid가 1개뿐이라 배열이 아니라 항목 하나짜리 dict로 오는 경우
            if isinstance(data, dict):
                wrapped = data.get("data") or data.get("items") or data.get("result")
                if isinstance(wrapped, list):
                    data = wrapped
                elif any(k in data for k in ("basePrice", "base_price", "sid", "currentStock")):
                    data = [data]  # 단일 항목 dict -> 리스트로 변환
                else:
                    raise RuntimeError(f"arsha API 응답이 예상 못한 dict 구조입니다. keys={list(data.keys())}")

            if not isinstance(data, list):
                raise RuntimeError(f"arsha API 응답 형식이 예상과 다릅니다. type={type(data)}")

            entries = []
            for row in data:
                try:
                    min_enh = row.get("sid", row.get("subKey", row.get("minEnhance", 0)))
                    entries.append({
                        "item_id": int(row.get("mainKey", row.get("id", item_id))),
                        "min_enhance": int(min_enh),
                        "max_enhance": int(row.get("maxEnhance", min_enh)),
                        "base_price": int(row.get("basePrice", row.get("base_price", 0))),
                        "current_stock": int(row.get("currentStock", row.get("current_stock", 0))),
                        "total_trades": int(row.get("totalTrades", row.get("total_trades", 0))),
                        "last_sale_price": int(row.get("lastSoldPrice", row.get("last_sale_price", 0))),
                    })
...
async def fetch_market_sublist(item_id):
    """
    가격 데이터 통합 조회: arsha.io를 먼저 시도하고, 실패하면 자동으로 기존 펄어비스(PA) 직통 API로 폴백한다.
    (arsha.io가 dump처럼 또 500을 뱉을 가능성에 대비해, 기존에 안정적으로 동작하던 PA 경로를 안전망으로 유지)
    """
    try:
        entries = await fetch_arsha_sublist(item_id)
        if entries:
            return entries
        raise RuntimeError("arsha API가 빈 결과를 반환함")
    except Exception as e:
        print(f"⚠️ arsha 가격 API 실패, PA 직통으로 폴백 (item_id={item_id}): {type(e).__name__}: {e}")
        return await fetch_pa_sublist(item_id)
...
def calculate_devour(start, target):
    """기존 '은은한 포식' 단가(5억) 기준의 단순 계산 — 하위 호환용으로 남겨둠"""
...
# ==========================================
# [포식 계산기 - 구간별 증가량 테이블]
# 어둠 포식(원본) 테이블은 공식 패치(2025-02-19) 기준 수치로 검증됨.
# 은은한 포식 테이블은 공개된 자료 기준 추정치이며, 정제된 포식은
# "어둠 포식과 동일 증가량, 2배 가격"으로 임시 가정함.
# 실제 게임 수치와 차이가 있을 수 있으니 확인되면 DEVOUR_GAIN_TABLES 값만 교체하면 됨.
# ==========================================
DEVOUR_GAIN_TABLES = {
    "어둠": [
    ...
    "은은한": [
        (100,104,8),(105,114,7),(115,125,6),(126,139,5),(140,167,4),(168,199,3),(200,249,2),(250,299,1)
    ],
}

def _devour_gain(stack, devour_type):
    """해당 스택에서 포식 1개 사용 시 증가하는 스택 수"""
    if devour_type == "정제된":
        # 정제된 포식: 어둠 포식과 동일한 증가량 테이블 사용 (가정)
        table = DEVOUR_GAIN_TABLES["어둠"]
    else:
...
def calculate_devour_type(start: int, target: int, devour_type: str):
    """포식 종류(어둠/은은한/정제된)별로 start->target까지 필요한 개수와 최종 스택을 계산"""
    cur, count = start, 0
    while cur < target:
        g = _devour_gain(cur, devour_type)
        cur += g
        count += 1
        if cur >= 300:
            cur = 300
...
class AvgPriceModal(discord.ui.Modal, title='아이템 평균가 조회'):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # 이름이 짧은(=더 정확히 일치하는) 순서로 정렬 후, 드롭다운 한도(25개)를 고려해 넉넉히 가져옴
        # (예: "활력의 비약" 검색 시 "활력의 비약", "샘솟는 활력의 비약", "[파티] 활력의 비약" 등
        #  비슷한 이름을 가진 아이템들이 전부 후보로 남도록 함)
        matches.sort(key=lambda m: len(m[0]))
        matches = matches[:15]
        
        async def fetch_item(iid):
            try:
                return await fetch_market_sublist(iid)
            except Exception as e:
...
# ==========================================
# 카프라스 계산기 장비 목록 (표시명: (CAPHRAS_DB 키, 등급))
CAPHRAS_EQUIP_OPTIONS = [
    ("동 우두머리 주무기/각성/보조", "녹색무기", "동"),
    ("동 우두머리 방어구",           "방어구",   "동"),
    ("동 초록 방어구",               "방어구",   "동"),
    ("동 초록 보조무기",             "보조무기", "동"),
]
...
class CaphrasStepModal(discord.ui.Modal, title='단계 입력'):
    ...
    def __init__(self, display_name: str, eq: str, gr: str):
        super().__init__()
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            start, target = int(self.start_lvl.value.strip()), int(self.target_lvl.value.strip())
        except ValueError:
            await interaction.response.send_message("숫자만 입력해 주세요!", ephemeral=True); return
        db = CAPHRAS_DB.get(self.eq, {}).get(self.gr)
        if not db:
            await interaction.response.send_message("장비 데이터를 찾을 수 없습니다.", ephemeral=True); return
        if start not in db or target not in db or start >= target:
            await interaction.response.send_message("올바른 단계 범위를 입력해 주세요.", ephemeral=True); return
        req_stones = db[target][1] - db[start][1]
        total_silver = req_stones * 2000000
        embed = discord.Embed(color=0x2ecc71, title="카프라스 계산기")
        embed.description = (
            f"**{self.display_name}** {start}단계에서 {target}단계:\n\n"
            f"필요한 카프라스: **{req_stones:,}개**\n"
            f"필요 금액: **{total_silver:,}** ({format_kr(total_silver)})"
        )
...
class CaphrasSelectView(discord.ui.View):
    def __init__(self):
        super().__init__(timeout=None)
        options = [
            discord.SelectOption(label=display, value=str(i))
            for i, (display, _, _) in enumerate(CAPHRAS_EQUIP_OPTIONS)
        ]
        self.select = discord.ui.Select(placeholder="장비를 선택해주세요.", options=options)
        self.select.callback = self.select_callback
        self.add_item(self.select)

    async def select_callback(self, interaction: discord.Interaction):
        idx = int(self.select.values[0])
        display_name, eq, gr = CAPHRAS_EQUIP_OPTIONS[idx]
        await interaction.response.send_modal(CaphrasStepModal(display_name, eq, gr))
...
# ==========================================
# [포식 계산기 - 거래소 포식(어둠/은은한/정제된) + 이벤트(희미한) + 효율계산]
# ==========================================
# 거래소 매물/가격이 안 불러와지던 원인: 아이템ID가 부정확했음 (특히 어둠 포식 766743 -> 65319로 정정).
# 안전하게 ITEM_LIST(실시간 아이템 DB, /아이템디버그로 검증 가능)에서 정확한 이름으로 우선 조회하고,
# 혹시 ITEM_LIST 로딩 전이거나 이름이 안 맞을 경우를 대비해 검증된 ID를 fallback으로 사용한다.
DEVOUR_ITEM_NAMES = {
    "어둠": "어둠 포식의 기원",
    "은은한": "은은한 어둠 포식의 기원",
    "정제된": "정제된 포식의 기원",
}
DEVOUR_ITEM_FALLBACK_IDS = {
    "어둠": 65319,     # 어둠 포식의 기원 (BDO Codex 확인됨)
    "은은한": 767102,  # 은은한 어둠 포식의 기원 (BDO Codex 확인됨)
    "정제된": 768000,  # 정제된 포식의 기원 (2026년 2월 신규 아이템, 정확한 ID 미확인 - ITEM_LIST 우선 사용 권장)
}

def get_devour_item_id(devour_type: str) -> int:
    """포식 종류 이름으로 ITEM_LIST에서 정확한 아이템ID를 우선 조회. 없으면 fallback ID 사용."""
    name = DEVOUR_ITEM_NAMES.get(devour_type)
    if name and name in ITEM_LIST:
        return ITEM_LIST[name]
    return DEVOUR_ITEM_FALLBACK_IDS.get(devour_type, 0)

# 포식 종류별 디스코드 커스텀 이모지
DEVOUR_EMOJIS = {
    "어둠": "<:dark_devour:1521373370506022993>",
    "이벤트": "<:event_devour:1521373451921920151>",
    "은은한": "<:faint_devour:1521373470519459881>",
    "정제된": "<:refined_devour:1521373484293423134>",
}

class DevourTradeModal(discord.ui.Modal):
    """어둠/은은한/정제된 포식 (거래소) - 현재스택, 희망스택 입력"""
    start_stack = discord.ui.TextInput(label='현재스택 (모험일지, 발크스의 외침 미포함!! 순수 스택 입력)', placeholder='100~299 숫자만 입력하세요.', required=True, max_length=3)
    target_stack = discord.ui.TextInput(label='희망스택 (101~300)', placeholder='101~300 숫자만 입력하세요.', required=True, max_length=3)

    def __init__(self, devour_type: str):
        super().__init__(title=f'{devour_type} 포식 (거래소)')
        self.devour_type = devour_type
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if not (101 <= target <= 300): await interaction.response.send_message("희망스택은 101~300 사이로 입력해 주세요.", ephemeral=True); return
        if start >= target: await interaction.response.send_message("희망스택은 현재스택보다 커야 합니다.", ephemeral=True); return

        await interaction.response.defer(ephemeral=True)
        count, final_stack = calculate_devour_type(start, target, self.devour_type)
        item_id = get_devour_item_id(self.devour_type)
        price, _, _ = await get_market_price(item_id) if item_id else (0, 0, 0)
        total_cost = count * price

        embed = discord.Embed(color=0x3498db, title=f"{DEVOUR_EMOJIS.get(self.devour_type, '')} {self.devour_type} 포식 (거래소)")
        ...
        if price > 0:
            embed.add_field(name="필요 금액", value=f"{total_cost:,} ({format_kr(total_cost)})", inline=True)
        else:
            embed.add_field(name="필요 금액", value="시세 조회 실패 (캐시 없음)", inline=True)
        await interaction.followup.send(embed=embed, ephemeral=True)


class DevourFaintModal(discord.ui.Modal):
    """희미한 포식 (이벤트) - 현재스택, 보유개수 입력 (1개당 1스택 고정)"""
...
class DevourEfficiencyModal(discord.ui.Modal):
    """어둠 포식 효율 계산 - 구간별로 가장 저렴한 포식 종류를 그리디로 추천"""
    start_stack = discord.ui.TextInput(label='현재스택 (모험일지, 발크스의 외침 미포함!! 순수 스택 입력)', placeholder='100~299 숫자만 입력하세요.', required=True, max_length=3)
    target_stack = discord.ui.TextInput(label='희망스택 (101~300)', placeholder='101~300 숫자만 입력하세요.', required=True, max_length=3)

    def __init__(self):
        super().__init__(title='어둠 포식 효율 계산')
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        dark_price, _, _ = await get_market_price(get_devour_item_id("어둠"))
        silver_price, _, _ = await get_market_price(get_devour_item_id("은은한"))
        refined_price, _, _ = await get_market_price(get_devour_item_id("정제된"))
        prices = {"어둠": dark_price, "은은한": silver_price, "정제된": refined_price}

        def unit_cost(devour_type, stack):
            price = prices.get(devour_type, 0)
            if price <= 0:
                return float('inf')
            g = _devour_gain(stack, devour_type)
            return price / g

        # 매 스택마다 가장 저렴한 포식 종류를 그리디로 선택
        cur = start
        type_counts = {"어둠": 0, "은은한": 0, "정제된": 0}
        segments = []  # (시작, 끝, 타입)
        seg_start, seg_type = cur, None

        while cur < target:
            candidates = {t: unit_cost(t, cur) for t in ("어둠", "은은한", "정제된")}
            best_type = min(candidates, key=lambda k: candidates[k])
            if candidates[best_type] == float('inf'):
                # 가격 정보가 전부 없으면 어둠 포식 기준으로 진행
                best_type = "어둠"
            if best_type != seg_type:
                if seg_type is not None:
                    segments.append((seg_start, cur, seg_type))
                seg_type, seg_start = best_type, cur
            g = _devour_gain(cur, best_type)
            cur += g
            type_counts[best_type] += 1
            if cur >= 300:
                cur = 300
                break
        segments.append((seg_start, min(cur, target if target < 300 else 300), seg_type))

        dark_count, silver_count, refined_count = type_counts["어둠"], type_counts["은은한"], type_counts["정제된"]
        dark_total = dark_count * dark_price
        silver_total = silver_count * silver_price
        refined_total = refined_count * refined_price
        grand_total = dark_total + silver_total + refined_total
        final_stack = min(cur, 300)

        embed = discord.Embed(color=0x2b2d31, title="어둠 포식 효율 계산")
        embed.add_field(name="시작 정보", value=f"시작 스택: {start}\n희망 스택: {target}\n최종 스택: {final_stack}", inline=False)

        icon = {"어둠": DEVOUR_EMOJIS["어둠"], "은은한": DEVOUR_EMOJIS["은은한"], "정제된": DEVOUR_EMOJIS["정제된"]}
        seg_lines = "\n".join([f"{s} → {e} : {icon[t]} {t} 포식" for s, e, t in segments if s != e])
        embed.add_field(name="구간별 효율적인 포식", value=seg_lines or "계산 불가 (가격 정보 없음)", inline=False)

        if dark_count > 0 or dark_price <= 0:
            embed.add_field(name=f"{DEVOUR_EMOJIS['어둠']} 어둠 포식 (거래소)", value=f"필요 개수: {dark_count}개\n비용: {dark_total:,} ({format_kr(dark_total)})", inline=False)
        if silver_count > 0 or silver_price <= 0:
            embed.add_field(name=f"{DEVOUR_EMOJIS['은은한']} 은은한 포식 (거래소)", value=f"필요 개수: {silver_count}개\n비용: {silver_total:,} ({format_kr(silver_total)})", inline=False)
        if refined_count > 0 or refined_price <= 0:
            embed.add_field(name=f"{DEVOUR_EMOJIS['정제된']} 정제된 포식 (거래소)", value=f"필요 개수: {refined_count}개\n비용: {refined_total:,} ({format_kr(refined_total)})", inline=False)

        embed.add_field(name="최종 결과", value=f"총 비용: {grand_total:,} ({format_kr(grand_total)})\n최종 스택: {final_stack}", inline=False)
        await interaction.followup.send(embed=embed, ephemeral=True)


class DevourSelectView(discord.ui.View):
    """포식 계산기 메인 패널: 어둠/은은한/정제된(거래소) + 희미한(이벤트) + 효율계산"""
    def __init__(self): super().__init__(timeout=180)

    @discord.ui.button(label="어둠 포식 (거래소)", emoji=DEVOUR_EMOJIS["어둠"], style=discord.ButtonStyle.success, row=0)
    async def btn_dark(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_modal(DevourTradeModal("어둠"))

    @discord.ui.button(label="은은한 포식 (거래소)", emoji=DEVOUR_EMOJIS["은은한"], style=discord.ButtonStyle.success, row=0)
    async def btn_silver(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_modal(DevourTradeModal("은은한"))

    @discord.ui.button(label="정제된 포식 (거래소)", emoji=DEVOUR_EMOJIS["정제된"], style=discord.ButtonStyle.success, row=0)
    async def btn_refined(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_modal(DevourTradeModal("정제된"))

    @discord.ui.button(label="희미한 포식 (이벤트)", emoji=DEVOUR_EMOJIS["이벤트"], style=discord.ButtonStyle.primary, row=1)
    async def btn_faint(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_modal(DevourFaintModal())

    @discord.ui.button(label="효율계산", style=discord.ButtonStyle.secondary, row=1)
    async def btn_efficiency(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_modal(DevourEfficiencyModal())
...
SOV_ITEM_IDS = {
    # 주무기
    "동 검은별 주무기":     {"id": 11756, "sid": 20},
    "동 크자카 주무기":     {"id": 11637, "sid": 20},
    # 각성무기
    "동 검은별 각성무기":   {"id": 11757, "sid": 20},
    "동 단텔리온 각성무기": {"id": 11882, "sid": 20},
    # 보조무기
    "동 검은별 보조무기":   {"id": 11758, "sid": 20},
    "동 누베르 보조무기":   {"id": 11638, "sid": 20},
    "동 쿠툼 보조무기":     {"id": 11639, "sid": 20},
    # 공통 재료
    "마력의 파편":          {"id": 44195,  "sid": 0},
    "카프라스의 돌":        {"id": 721003, "sid": 0},
    "황혼의 보석":          {"id": 16001,  "sid": 0},
    "태초의 보석":          {"id": 16002,  "sid": 0},
}

# 조합 레시피 정의: (표시이름, [(아이템명, 수량), ...])
SOV_RECIPES = {
    "주무기": [
        ("동 검은별 주무기 + 동 검은별 주무기",
         [("동 검은별 주무기", 2), ("마력의 파편", 1), ("카프라스의 돌", 1)]),
        ("동 검은별 주무기 + 동 크자카 주무기 + 황혼의 보석",
         [("동 검은별 주무기", 1), ("동 크자카 주무기", 1), ("마력의 파편", 1), ("카프라스의 돌", 1), ("황혼의 보석", 1)]),
    ],
    "각성무기": [
        ("동 검은별 각성무기 + 동 검은별 각성무기",
         [("동 검은별 각성무기", 2), ("마력의 파편", 1), ("카프라스의 돌", 1)]),
        ("동 검은별 각성무기 + 동 단텔리온 각성무기 + 황혼의 보석",
         [("동 검은별 각성무기", 1), ("동 단텔리온 각성무기", 1), ("마력의 파편", 1), ("카프라스의 돌", 1), ("황혼의 보석", 1)]),
    ],
    "보조무기": [
        ("동 검은별 보조무기 + 동 검은별 보조무기 + 황혼의 보석",
         [("동 검은별 보조무기", 2), ("마력의 파편", 1), ("카프라스의 돌", 1), ("황혼의 보석", 1)]),
        ("동 검은별 보조무기 + 동 누베르/쿠툼 보조무기",
         [("동 검은별 보조무기", 1), ("동 누베르 보조무기", 1), ("동 쿠툼 보조무기", 1), ("마력의 파편", 1), ("카프라스의 돌", 1)]),
        ("동 검은별 + 황혼의 보석 + 태초의 보석",
         [("동 검은별 보조무기", 1), ("마력의 파편", 1), ("카프라스의 돌", 1), ("황혼의 보석", 1), ("태초의 보석", 1)]),
    ],
}

async def fetch_sov_prices(weapon_type: str) -> dict:
    """해당 무기 타입의 조합에 필요한 모든 아이템 가격을 한 번에 조회"""
    needed = set()
    for _, ingredients in SOV_RECIPES[weapon_type]:
        for name, _ in ingredients:
            needed.add(name)
    prices = {}
    tasks = {name: get_market_price(SOV_ITEM_IDS[name]["id"], SOV_ITEM_IDS[name]["sid"]) for name in needed if name in SOV_ITEM_IDS}
...
class SovResultView(discord.ui.View):
    """계산 결과 + 평균가 확인 버튼"""
    def __init__(self, weapon_type: str, prices: dict):
        super().__init__(timeout=None)
        self.weapon_type = weapon_type
        self.prices = prices

    def build_embed(self) -> discord.Embed:
        embed = discord.Embed(title="군왕 무기 계산기", color=0xffd700)
        lines = [f"**군왕 무기 계산({self.weapon_type})**\n"]
        for recipe_name, ingredients in SOV_RECIPES[self.weapon_type]:
            total = sum(self.prices.get(name, (0,))[0] * qty for name, qty in ingredients)
            lines.append(f"**{recipe_name}**")
            lines.append(f"┃ {format_kr(total)}\n")
        embed.description = "\n".join(lines)
        return embed
    ...
    async def btn_avg(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        lines = []
        shown = {}
        for _, ingredients in SOV_RECIPES[self.weapon_type]:
            for name, _ in ingredients:
                if name in shown or name not in SOV_ITEM_IDS:
                    continue
                shown[name] = True
                price, _, _ = self.prices.get(name, (0, 0, 0))
...
class SovWeaponSelectView(discord.ui.View):
    ...
    async def _calc(self, interaction: discord.Interaction, weapon_type: str):
        await interaction.response.defer(ephemeral=True)
        prices = await fetch_sov_prices(weapon_type)
        view = SovResultView(weapon_type, prices)
    ...
    async def btn_main(self, interaction: discord.Interaction, button: discord.ui.Button):
        await self._calc(interaction, "주무기")

    @discord.ui.button(label="각성무기", style=discord.ButtonStyle.secondary)
    async def btn_awk(self, interaction: discord.Interaction, button: discord.ui.Button):
        await self._calc(interaction, "각성무기")

    @discord.ui.button(label="보조무기", style=discord.ButtonStyle.secondary)
    async def btn_sub(self, interaction: discord.Interaction, button: discord.ui.Button):
        await self._calc(interaction, "보조무기")
...
# [어둠의 틈 시스템]
# ==========================================
def schedule_ephemeral_delete(interaction: discord.Interaction, delay: int = 180):
    """버튼 클릭자에게만 보이는(ephemeral) 응답 메시지를 delay초(기본 3분) 뒤 자동 삭제한다."""
...
class DarkRiftActionView(discord.ui.View):
    ...
    @discord.ui.button(label="현재 시간으로 등록", style=discord.ButtonStyle.success, custom_id="rift_btn_now")
    async def btn_now(self, interaction: discord.Interaction, button: discord.ui.Button):
        conn = sqlite3.connect('bdo_data.db')
        c = conn.cursor()
        c.execute("SELECT kill_time, reporter_name FROM rift_history WHERE id=1")
        existing = c.fetchone()
        if existing:
            conn.close()
            kill_time = datetime.strptime(existing[0], '%Y-%m-%d %H:%M:%S').replace(tzinfo=KST)
            spawn_time = kill_time + timedelta(days=5)
            embed = build_rift_status_embed(spawn_time, reporter_name=existing[1])
            embed.description = "⚠️ 이미 제보된 정보가 있습니다.\n\n" + (embed.description or "")
            await interaction.response.send_message(embed=embed, ephemeral=True)
        ...
        c.execute("INSERT OR REPLACE INTO rift_history (id, reporter_id, reporter_name, kill_time) VALUES (1, ?, ?, ?)",
                  (interaction.user.id, interaction.user.display_name, now.strftime('%Y-%m-%d %H:%M:%S')))
        conn.commit(); conn.close()
    ...
    async def btn_manual(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if existing:
            ...
            schedule_ephemeral_delete(interaction)
            return
        await interaction.response.send_modal(DarkRiftManualModal())
...
STATUS_SERVER_REGIONS = ["발레노스", "세렌디아", "칼페온", "메디아", "발렌시아", "울루키타", "에다니아"]
STATUS_SERVER_NUMBERS = ["1", "2", "3"]
STATUS_DEFAULT_MINUTES = {"blessing": 180, "edana": 60}  # 축복=3시간, 에다니아=1시간
STATUS_TITLES = {"blessing": "아침의 축복", "edana": "에다니아"}


async def get_status_notify_channel(s_type, fallback_channel):
    """타입별(blessing/edana)로 지정된 알림 채널을 가져온다. 지정되지 않았으면 fallback_channel(패널이 있는 채널) 사용."""
    conn = sqlite3.connect('bdo_data.db')
    c = conn.cursor()
...
async def finalize_status_report(interaction: discord.Interaction, s_type, kind, server, minutes):
    """제보 내용을 DB에 저장하고, 지정된 채널에 공개 알림을 보낸 뒤 3분 후 자동 삭제, DM 알림 발송까지 처리하는 공용 함수."""
    ...
    # 2) 지정된 채널(없으면 패널이 있는 채널)에 누가/어디인지 공개 알림 (3분 뒤 자동 삭제)
...
class StatusTimeSelectView(discord.ui.View):
    """마지막 단계: 기본 시간 버튼(축복=3시간/에다니아=1시간) + 직접 입력 버튼"""
    def __init__(self, s_type, kind, server):
...
class StatusServerSelectView(discord.ui.View):
    """두 번째 단계: 지역 선택 버튼 (발레노스/세렌디아/칼페온/메디아/발렌시아/울루키타/에다니아)"""
...
class StatusKindSelectView(discord.ui.View):
    """첫 번째 단계: 종류 선택 버튼 (해축/달축/땅축 또는 에다니아 5종)"""
...
class StatusViewPanel(discord.ui.View):
    def __init__(self, s_type, s_title):
        ...
        btn_refresh = discord.ui.Button(label="위치 현황 보기", style=discord.ButtonStyle.primary, custom_id=f"btn_refresh_{s_type}")
        btn_refresh.callback = self.btn_refresh_callback
        self.add_item(btn_refresh)

    async def btn_report_callback(self, interaction: discord.Interaction):
        embed = discord.Embed(title=f"{self.s_title} 제보", description="종류를 선택해주세요.", color=0x2b2d31)
        await interaction.response.send_message(embed=embed, view=StatusKindSelectView(self.s_type), ephemeral=True)

    async def btn_refresh_callback(self, interaction: discord.Interaction):
        rows = _get_status_rows(self.s_type)
        embed = build_status_view_embed(self.s_type, self.s_title, rows)
        await interaction.response.send_message(embed=embed, ephemeral=True)

        # 1분마다 자동 갱신 (최대 3시간, 만료된 제보 없으면 조기 종료)
        msg = await interaction.original_response()
        for _ in range(180):
            await asyncio.sleep(60)
            try:
                new_rows = _get_status_rows(self.s_type)
                new_embed = build_status_view_embed(self.s_type, self.s_title, new_rows)
...
class QnaAdminView(discord.ui.View):
    ...
    async def btn_reply(self, interaction: discord.Interaction, button: discord.ui.Button):
        if not interaction.user.guild_permissions.administrator and not interaction.user.get_role(ADMIN_ROLE_ID):
            await interaction.response.send_message("관리자만 누를 수 있습니다.", ephemeral=True)
            return
        await interaction.response.send_modal(QnaReplyModal(self.target_user_id))

    @discord.ui.button(label="채널 닫기", style=discord.ButtonStyle.danger, custom_id="qna_close_btn")
    async def btn_close(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        try:
            await interaction.channel.delete(reason="문의/건의 채널 닫기")
        except:
            pass
...
class AnonAdminView(discord.ui.View):
    ...
    @discord.ui.button(label="답변 전송", style=discord.ButtonStyle.success, custom_id="anon_reply_btn")
    async def btn_reply(self, interaction: discord.Interaction, button: discord.ui.Button):
        if not interaction.user.guild_permissions.administrator and not interaction.user.get_role(ADMIN_ROLE_ID):
            await interaction.response.send_message("관리자만 누를 수 있습니다.", ephemeral=True)
            return
        await interaction.response.send_modal(AnonReplyModal(self.target_user_id))

    @discord.ui.button(label="채널 닫기", style=discord.ButtonStyle.danger, custom_id="anon_close_btn")
    async def btn_close(self, interaction: discord.Interaction, button: discord.ui.Button):
        if not interaction.user.guild_permissions.administrator and not interaction.user.get_role(ADMIN_ROLE_ID):
            await interaction.response.send_message("관리자만 누를 수 있습니다.", ephemeral=True)
            return
        ...
        await asyncio.sleep(5)
        try:
            await interaction.channel.delete(reason="익명 제보 채널 닫기")
        except:
            pass
...
class SetupBossView(discord.ui.View):
    ...
    async def btn_time(self, interaction: discord.Interaction, button: discord.ui.Button):
        today_str = WEEKDAYS[datetime.now(KST).weekday()][0]
        embed = discord.Embed(title=f"{today_str}요일 월드 보스 시간표 (오늘)", color=0xf39c12)
        embed.description = "".join([f"**{t}** : {b}\n\n" for t, b in BOSS_DB[today_str]])
        embed.set_footer(text="아래 드롭다운에서 다른 요일도 확인할 수 있습니다.")
        await interaction.response.send_message(embed=embed, view=BossDaySelectView(), ephemeral=True)
...
class SetupUtilityView(discord.ui.LayoutView):
    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        ...
        if cid == "util_cap":
            embed = discord.Embed(title="카프라스 계산기", description="장비를 선택해주세요.", color=0x2ecc71)
            await interaction.response.send_message(embed=embed, view=CaphrasSelectView(), ephemeral=True)
        elif cid == "util_apdp": await interaction.response.send_modal(ApDpModal())
        elif cid == "util_devour":
            embed = discord.Embed(title="포식 계산기", description="아래 버튼을 클릭해주세요.", color=0x2b2d31)
            await interaction.response.send_message(embed=embed, view=DevourSelectView(), ephemeral=True)
        elif cid == "util_sov": 
            ...
            embed = discord.Embed(title="데키아 등불", color=0xf1c40f)
            desc = ""
            ...
            ranking.sort(key=lambda x: x["unit_price"])
            embed = discord.Embed(title="영물의 축복", color=0x2ecc71)
            desc = ""
            ...
            if row:
                ...
                spawn_time = kill_time + timedelta(days=5)
                embed = build_rift_status_embed(spawn_time, reporter_name=row[1])
            else:
                embed = discord.Embed(title="어둠의 틈", description="기록된 처치 시간이 없습니다.\n아래 버튼으로 등록해주세요.", color=0x2b2d31)
            await interaction.response.send_message(embed=embed, view=DarkRiftActionView(), ephemeral=True)
            schedule_ephemeral_delete(interaction)
        elif cid == "util_snipe": 
...
class OfficialLinksView(discord.ui.View):
    def __init__(self):
        super().__init__(timeout=None)
        links = [
            ("공지사항", "https://www.kr.playblackdesert.com/ko-KR/News/Notice?boardType=1 "),
            ("업데이트", "https://www.kr.playblackdesert.com/ko-KR/News/Notice?boardType=2"),
            ("이벤트", "https://www.kr.playblackdesert.com/ko-KR/News/Notice?boardType=3"),
            ("펄상점", "https://payment.kr.playblackdesert.com/ko-KR/Pay/Home/?_gl=1*zz86sk*_gcl_au*MTY4MTA1NDczNC4xNzgyNTM3NzI1&_ga=2.263919813.813103790.1782537726-1813507788.1782537726 "),
            ("공식 홈페이지", "https://www.kr.playblackdesert.com/"),
            ("연구소", "https://www.global-lab.playblackdesert.com/"),
            ("쿠폰 확인", "https://www.kr.playblackdesert.com/ko-KR/News/Detail?groupContentNo=10748&countryType=ko-KR"),
            ("최신 영상", "https://www.youtube.com/@BlackDesert_KR"),
            ("검은사막 인벤", "https://black.inven.co.kr/"),
            ("검통디", "https://korbdo.co.kr/"),
            ("제작노트", "https://korbdo.co.kr/#craft"),
            ("광명석조합식", "https://www.kr.playblackdesert.com/ko-KR/Wiki?wikiNo=273#t1"),
        ]
...
async def rift_alert_loop():
    ...
    conn = sqlite3.connect('bdo_data.db')
    c = conn.cursor()
    c.execute("SELECT reporter_id, kill_time FROM rift_history WHERE id=1")
    row = c.fetchone()
    conn.close()
    if not row:
        return
    user_id, kill_time_str = row
    try:
...
# [코어 명령어 및 봇 구동]
# ==========================================
DUMP_URL = "https://api.arsha.io/util/db/dump?lang=kr"
DUMP_HEADERS = {"User-Agent": "Mozilla/5.0", "Accept": "application/json"}
DUMP_MAX_RETRIES = 3
DUMP_RETRY_DELAY_SEC = 10  # 재시도 사이 대기 시간(초)
DUMP_BACKUP_FRESH_HOURS = 24  # 백업 파일이 이 시간 이내면 dump API를 아예 호출하지 않음 (force=True면 무시)

async def load_item_dump(force: bool = False) -> str:
    """
    아이템 이름->ID 덤프(ITEM_LIST)를 로드한다.
    force=False(기본)면 로컬 백업이 DUMP_BACKUP_FRESH_HOURS 이내일 때 API 호출 자체를 건너뛴다.
    (재시작마다 무거운 dump 엔드포인트를 매번 때리는 게 arsha.io 쪽 500의 주요 원인으로 보여서,
     불필요한 호출 자체를 줄여 부하를 낮추기 위함)
    force=True면 캐시 신선도와 무관하게 무조건 새로 받아온다 (수동 갱신용).
    결과 요약 메시지(str)를 반환한다.
    """
    global ITEM_LIST

    if not force and os.path.exists('items_v2.json'):
        age_sec = datetime.now().timestamp() - os.path.getmtime('items_v2.json')
        age_hours = age_sec / 3600
        if age_hours < DUMP_BACKUP_FRESH_HOURS:
            with open('items_v2.json', 'r', encoding='utf-8') as f:
                backup = json.load(f)
                ITEM_LIST.update(backup)
            msg = f"⏩ 로컬 백업이 {age_hours:.1f}시간 전 데이터로 충분히 최신이라 dump API 호출을 건너뜁니다. ({len(backup)}개 로드됨, 현재 ITEM_LIST 총 {len(ITEM_LIST)}개)"
            print(msg)
            return msg
    ...
    for attempt in range(1, DUMP_MAX_RETRIES + 1):
        try:
            async with aiohttp.ClientSession(headers=DUMP_HEADERS, connector=aiohttp.TCPConnector(ssl=False)) as session:
                async with session.get(DUMP_URL, timeout=aiohttp.ClientTimeout(total=60)) as response:
                    if response.status == 200:
                        data = await response.json()
                        if not isinstance(data, list) or len(data) == 0:
                            raise RuntimeError(f"dump 응답이 비어있거나 형식이 예상과 다릅니다. type={type(data)}")
                        ...
                        msg = f"✅ 총 {len(ITEM_LIST)}개의 아이템 연동 완료! (dump {len(data)}건 중 {skipped}건 스킵, 시도 {attempt}/{DUMP_MAX_RETRIES}회차에 성공)"
                        print(msg)
                        dump_succeeded = True
                        break
                    else:
                        body_preview = (await response.text())[:300]
                        raise RuntimeError(f"dump API status {response.status}, body={body_preview}")
        except Exception as e:
            last_error = e
            print(f"⚠️ dump 로드 시도 {attempt}/{DUMP_MAX_RETRIES} 실패: {type(e).__name__}: {e}")
            if attempt < DUMP_MAX_RETRIES:
                print(f"   {DUMP_RETRY_DELAY_SEC}초 후 재시도합니다...")
                await asyncio.sleep(DUMP_RETRY_DELAY_SEC)

    if dump_succeeded:
        return msg

    print(f"⚠️ {DUMP_MAX_RETRIES}회 모두 dump 로드 실패. 마지막 오류: {type(last_error).__name__}: {last_error}")
    if os.path.exists('items_v2.json'):
        with open('items_v2.json', 'r', encoding='utf-8') as f:
            backup = json.load(f)
            ITEM_LIST.update(backup)
        msg = f"↩️ {DUMP_MAX_RETRIES}회 모두 실패하여 로컬 백업(items_v2.json) 사용: {len(backup)}개 로드됨. 현재 ITEM_LIST 총 {len(ITEM_LIST)}개\n마지막 오류: {type(last_error).__name__}: {last_error}"
    else:
        msg = "❌ dump 로드 실패 + 로컬 백업(items_v2.json)도 없어서 HARDCODED 9개만 사용 중입니다."
    print(msg)
    return msg
...
@bot.tree.command(name="제보채널설정", description="[관리자] 축복/에다니아 제보가 도착할 채널을 지정합니다")
@discord.app_commands.default_permissions(administrator=True)
@discord.app_commands.describe(종류="축복 또는 에다니아", 채널="제보 메시지가 도착할 채널")
@discord.app_commands.choices(종류=[
    discord.app_commands.Choice(name="아침의 축복", value="blessing"),
    discord.app_commands.Choice(name="에다니아", value="edana"),
])
async def set_status_notify_channel(interaction: discord.Interaction, 종류: discord.app_commands.Choice[str], 채널: discord.TextChannel):
    conn = sqlite3.connect('bdo_data.db')
    c = conn.cursor()
    c.execute("INSERT OR REPLACE INTO status_notify_channels (s_type, channel_id) VALUES (?, ?)", (종류.value, 채널.id))
    conn.commit(); conn.close()
    await interaction.response.send_message(f"✅ **{종류.name}** 제보가 이제 {채널.mention} 채널로 도착합니다.", ephemeral=True)
...
@bot.tree.command(name="아이템디비갱신", description="[관리자] 아이템 DB(이름→ID)를 arsha.io에서 강제로 새로 받아옵니다 (캐시 신선도 무시)")
@discord.app_commands.default_permissions(administrator=True)
async def refresh_item_db(interaction: discord.Interaction):
    await interaction.response.defer(ephemeral=True)
    result = await load_item_dump(force=True)
    await interaction.followup.send(make_codeblock(result), ephemeral=True)
